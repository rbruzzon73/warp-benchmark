...
#                                                                                #
#         Usage: ./warp-benchmark-v5.sh [mixed|standard|get|put|all]             #
#                                                                                #
############################# rbruzzon@redhat.com ################################
...
start_benchmark () {
    ...
    echo "Noobaa Deduplication: $DEDUPLICATION_DISABLE - Noobaa Endpoints: $WARP_CONCURRENT"
...
if [ "$STORAGE_CLASS" = "openshift-storage.noobaa.io" ] && [ "$DEDUPLICATION_DISABLE" = "true" ]; then
  ...
  echo "Waiting for all endpoints to clear the environment variable..."
  ...
  while true; do
    ...
    for pod in $POD_LIST; do
        # Extract the value; if it's removed, VALUE will be empty
        VALUE=$(oc get pod "$pod" -n openshift-storage -o jsonpath='{.spec.containers[*].env[?(@.name=="'$SEARCH_TERM'")].value}')
    ...
    if [ "$MATCH_COUNT" -eq "$ACTUAL_COUNT" ]; then
        echo "SUCCESS - Variable '$SEARCH_TERM' has been removed from all $MATCH_COUNT pods."
        break
    else
        echo "Pending - Variable still exists in $((ACTUAL_COUNT - MATCH_COUNT)) pods. Retrying in ${CHECK_INTERVAL}s..."
        sleep "$CHECK_INTERVAL"
    fi
...
creation_phase () {
    ...
    PHASE=$(oc get ns "$NAMESPACE" -o jsonpath='{.status.phase}' 2>/dev/null || echo "NotFound")
    
    if [ "$PHASE" == "Terminating" ]; then
        echo "Namespace $NAMESPACE is Terminating. Waiting for cleanup..."
        while oc get ns "$NAMESPACE" >/dev/null 2>&1; do sleep 2; done
    fi

    # Try to create, if it fails, just switch
    if ! oc new-project "$NAMESPACE" >/dev/null 2>&1; then
        echo "Namespace '$NAMESPACE' already exists. Switching context..."
...
run_get () {
    ...
    oc exec -n $NAMESPACE warp-runner -- /bin/sh -c '
	  ...
	  --obj.generator random \
          ...
          --insecure \
	  ${EXTRA} \
          --benchdata "$GET_FILE"> /proc/1/fd/1 2>&1 
    
        echo ">>> Analyzing GET Results:"
        ./warp analyze --analyze.v "${GET_FILE}.json.zst"
        ./warp analyze --analyze.v "${GET_FILE}.json.zst"> /proc/1/fd/1 2>&1
    '
...
run_put () {
    ...
    oc exec -n $NAMESPACE warp-runner -- /bin/sh -c '
	  ...
	  --obj.generator random \
          ...
          --insecure \
	  ${EXTRA} \
          --benchdata "$PUT_FILE"> /proc/1/fd/1 2>&1
    
        echo ">>> Analyzing PUT Results:"
        ./warp analyze --analyze.v "${PUT_FILE}.json.zst"
        ./warp analyze --analyze.v "${PUT_FILE}.json.zst"> /proc/1/fd/1 2>&1
    '
}
...
run_mixed () {
    ...
    oc exec -n $NAMESPACE warp-runner -- /bin/sh -c '
	  ...
	  --obj.generator random \
          ...
          --insecure \
	  ${EXTRA} \
          --benchdata "$MIXED_FILE"> /proc/1/fd/1 2>&1

        echo ">>> Analyzing MIXED Results:"
        ./warp analyze --analyze.v "${MIXED_FILE}.json.zst"
        ./warp analyze --analyze.v "${MIXED_FILE}.json.zst"> /proc/1/fd/1 2>&1
    '
...
benchmark_end () {
    ...
    echo "---------------------------------------------------"
    echo "[15/15] Benchmarks Completed."
    echo "---------------------------------------------------"
    echo ""
    echo "-------------------- NOTES ------------------------"
    echo "---------------------------------------------------"
